...
export const getAssayID = (req: NextApiRequest) : number => {
    return req.query.assayId? Number(req.query.assayId.toString()) : INVALID_ASSAY_ID;
}

export const INVALID_ASSAY_TYPE_ID = -1;
export const getAssayTypeID = (req: NextApiRequest) : number => {
    return req.query.assayTypeId? Number(req.query.assayTypeId.toString()) : INVALID_ASSAY_TYPE_ID;
}
