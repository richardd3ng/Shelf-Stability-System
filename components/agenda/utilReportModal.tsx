    ...
    return (
        <CloseableModal open={isChoosingDates} closeFn={() => setIsChoosingDates(false)} title="Lab Utilization Report">
            <Stack direction="column" gap={3}>
                ...
                <Button disabled={!canSubmit} variant="contained" color="primary" style={{textTransform : "none"}} onClick={() => {
                    fetchUtilizationData({startDate, endDate}).then((rows : UtilizationReportRow[]) => {
                        generateExcelUtilizationReport(rows, startDate, endDate);
                    }).catch((error) => {
                        console.log(error);
                    })
                }}>
