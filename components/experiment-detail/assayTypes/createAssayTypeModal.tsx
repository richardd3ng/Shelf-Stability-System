...
export const CreateAssayTypeModal : React.FC = () => {
    ...
    const fields : Field[] = [
        ...
        {field : description, setField : setDescription, label : "Description", required : false},
        {field : units, setField : setUnits, label : "Units", required : false}
    ];
    const {mutate : createType} = useMutationToCreateCustomAssayType();

    return (
        <CloseableModal open={isCreating} closeFn={() => setIsCreating(false)} title="Create Custom Assay Type">
            <Stack gap="3" direction="column">
                {fields.map((field, index) => (
                    <TextField key={index} value={field.field} required={field.required} label={field.label} style={{marginBottom : 8}} onChange={(e) => {
                        field.setField(e.target.value);
                    }}/>
                ))}


                <Button variant="contained" color="primary" disabled={name.length < 1} onClick={() => {
                    createType({
