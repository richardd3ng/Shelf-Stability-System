    ...
    const fetchDataAndSort = async () => {
      ...
      const sortedData = mockData.sort((a, b) => {
        switch (sortState.field) {
          ...
          case SortField.Week:
            return sortState.ascending ? a.week - b.week : b.week - a.week;
          default:
            return 0;
        }
      });
      ...
      >
        <ArrowUpward
          sx={{
            ...
            transform: sortState.ascending ? "None" : "rotate(180deg)",
          }}
        />
      </IconButton>
    );
    ...
    return (
      ...
      >
        <Container sx={{ width: "50%" }}>
          <SearchBar placeholder="Search Keyword" onEnter={handleSearch} />
        </Container>
        <Box sx={{ width: "25%", display: "flex", alignItems: "center" }}>
          ...
          <Select
            ...
            sx={{ height: "30px", width: "50%", marginLeft: "12px" }}
          >
            <MenuItem value={SortField.Id}>{SortField.Id}</MenuItem>
            <MenuItem value={SortField.Title}>{SortField.Title}</MenuItem>
            <MenuItem value={SortField.StartDate}>
              {SortField.StartDate}
            </MenuItem>
            <MenuItem value={SortField.Week}>{SortField.Week}</MenuItem>
          </Select>
          <SortArrow />
        </Box>
        <Button
